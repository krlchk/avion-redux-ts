    ...
    <footer className="mobile:gap-12 flex w-full flex-col gap-20 bg-[#F6F4F2] pt-14 pb-5 text-black">
      <Container className="mobile:flex-col mobile:items-start flex items-center justify-between gap-12">
        <div className="mobile:w-full w-1/2">
          <p className="xs:text-xl xs:leading-7 text-2xl leading-9 font-bold">
        ...
        <form className="mobile:w-full xs:flex-col flex w-1/2 gap-2">
          <input
            className="xs:text-base w-full border border-[#DEE2E6] bg-white px-4 py-2 text-xl font-medium text-black placeholder:text-[#DEE2E6]"
            type="email"
            placeholder="Enter your email"
          />
          <button className="xs:w-full xs:px-4 xs:text-base bg-[#947458] px-14 py-2 text-xl font-medium whitespace-nowrap text-white">
            Subscribe
          </button>
        </form>
      </Container>
      <Container className="tablet:grid-cols-2 mobile:gap-y-10 xs:grid-cols-1 grid grid-cols-4 gap-10">
        <div className="flex flex-col items-start justify-center gap-5 text-sm font-medium text-black/60">
          <div className="flex gap-2">
            <Logo />
            <h2 className="text-xl leading-6 font-bold text-black">Avion</h2>
          </div>
...
const FooterLinksColumn = ({ title, links }: FooterLinksColumnProps) => {
  return (
    <div className="flex flex-col items-start justify-start gap-3 text-sm font-medium text-black/60">
      <h2 className="mb-2 text-xl leading-6 font-bold text-black">{title}</h2>
