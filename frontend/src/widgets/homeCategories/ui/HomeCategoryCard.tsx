...
export const HomeCategoryCard = ({ category }: HomeCategoryCardProps) => {
  ...
  return (
    <div
      className="group tablet:min-h-96 mobile:min-h-64 xs:min-h-60 relative min-h-72 cursor-pointer overflow-hidden border-2 border-white bg-cover bg-center shadow-[0_24px_60px_rgba(0,0,0,0.24)] transition-all duration-500 hover:-translate-y-2 hover:border-white/40 hover:shadow-[0_34px_90px_rgba(0,0,0,0.46)]"
    ...
    >
      ...
      <div className="mobile:p-4 absolute inset-x-0 bottom-0 translate-y-5 p-5 opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
        <h3 className="mobile:text-lg text-xl leading-6 font-bold text-white">
          {productsCount} products
        </h3>
      </div>
      <p className="mobile:text-lg absolute top-3 left-3 px-3 py-1 text-xl font-bold text-white">
        {name}
      </p>
    </div>
  );
